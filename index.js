...
app.post('/check-availability', async (req, res) => {
  ...
  if (!toolCall) {
    return res.status(400).json({ error: 'No toolCall encontrado' });
  }
  ...
  try {
    ...
    return res.json({
      results: [{ toolCallId: toolCall.id, result: { ok: false, error: 'Invalid arguments' } }]
    });
  }
  ...
  try {
    ...
    console.log("Google Calendar tardó:", Date.now() - t0, "ms");
    ...
    return res.json({
      results: [{
        ...
        result: {
          ...
          roomType, inventory,
          range: { startDate, endDate },
          nights: [], maxReserved: 0,
          minAvailable: inventory,
          isAvailableForEntireRange: true
        }
      }]
    });
  }
